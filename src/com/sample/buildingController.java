...
//        }
        ...
        try (Connection conn = PooledConnection.getConnection()) {
            String sql;
                        if(archLocId != null && !archLocId.isEmpty()){
                            sql = "UPDATE FMO_ITEM_LOCATIONS SET ARCHIVED_FLAG = 2 WHERE ITEM_LOC_ID = ?";
                        }else{
                            sql = "UPDATE FMO_ITEM_LOCATIONS SET NAME = ?, DESCRIPTION = ?, IMAGE = ? WHERE ITEM_LOC_ID = ?";
                        }
                        ...
                        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
                            if(archLocId != null && !archLocId.isEmpty()){
                                ...
                                if (rowsUpdated > 0) {
                                    ...
                                    // Now update or insert into FMO_ITEM_LOC_MAP table
                                    if (!isNullOrEmpty(mapCoord)) {
                                        ...
                                        if (coords.length == 2) {
                                            ...
                                            String sqlMapUpdate = "UPDATE FMO_ITEM_LOC_MAP SET LATITUDE = ?, LONGITUDE = ? WHERE ITEM_LOC_ID = ?";
                                            try (PreparedStatement stmtMapUpdate = conn.prepareStatement(sqlMapUpdate)) {
                                                ...
                                                if (mapRowsUpdated == 0) {
                                                    ...
                                                    String sqlMapInsert = "INSERT INTO FMO_ITEM_LOC_MAP (ITEM_LOC_ID, LATITUDE, LONGITUDE) VALUES (?, ?, ?)";
