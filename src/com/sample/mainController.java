...
public class mainController extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try (
             ...
             PreparedStatement stmntFloor = con.prepareCall("SELECT * FROM FMO_ITEM_LOC_FLOORS ORDER BY ITEM_LOC_ID, CASE WHEN REGEXP_LIKE(NAME, '^[0-9]+F') THEN TO_NUMBER(REGEXP_SUBSTR(NAME, '^[0-9]+')) ELSE 9999 END, NAME");
             ...
             PreparedStatement stmntMaintSched = con.prepareCall("SELECT * FROM FMO_ITEM_MAINTENANCE_SCHED WHERE ACTIVE_FLAG = 1 AND ARCHIVED_FLAG = 1 ORDER BY ITEM_MS_ID");
             PreparedStatement stmntMaintType = con.prepareCall("SELECT * FROM FMO_ITEM_MAINTENANCE_TYPES");
             ...
             PreparedStatement stmntDUsers = con.prepareCall("SELECT * FROM FMO_ITEM_DUSERS ORDER BY USER_ID");
             PreparedStatement stmntQuotations = con.prepareCall("SELECT * FROM FMO_ITEM_QUOTATIONS ORDER BY QUOTATION_ID");
             PreparedStatement stmntToDo = con.prepareCall("SELECT * FROM FMO_TO_DO_LIST");
             PreparedStatement stmntMap = con.prepareCall("SELECT * FROM FMO_ITEM_LOC_MAP");
                            ...
                            if (itemHIDParam != null && !itemHIDParam.isEmpty()) {
                                ...
                                try (Connection conn = PooledConnection.getConnection();
                                     PreparedStatement stmt = conn.prepareStatement(
                                         ...
                                         "FROM FMO_MAINTENANCE_ASSIGN ma " +
                                         "JOIN FMO_ITEM_DUSERS u ON ma.USER_ID = u.USER_ID " +
