...
//        System.out.println("tdAction: " + tdAction);
        ...
        try (Connection conn = PooledConnection.getConnection()) {
            ...
            if ("delete".equals(tdAction)) {
                sql = "DELETE FROM FMO_TO_DO_LIST WHERE LIST_ITEM_ID = ?";
            }else if ("check".equals(tdAction)) {
                sql = "UPDATE FMO_TO_DO_LIST SET IS_CHECKED = 1 WHERE LIST_ITEM_ID = ?";
            }else if ("uncheck".equals(tdAction)) {
                sql = "UPDATE FMO_TO_DO_LIST SET IS_CHECKED = 0 WHERE LIST_ITEM_ID = ?";
            }else{
                sql = "INSERT INTO FMO_TO_DO_LIST (EMP_NUMBER, LIST_CONTENT, START_DATE, END_DATE) VALUES (?, ?, ?, ?)";
            }
